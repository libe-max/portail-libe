    ...
    <section class="section" v-show="currentTab === tabs[0]">
      <div class="container">
        <div class="disclaimer" style="text-align: center; margin-bottom: 40px;">
          <a href="http://10.14.244.101:8082/" target="_blank" style="border-bottom: 2px solid #e61e49;">Cliquez ici</a> pour le LibéStories (générateur de Stories Instagram) disponible dans la V2 de la boîte à outils
        </div>
...
export default {
  ...
  methods: {
    ...
    urlImg (id, image) {
      ...
      else return 'http://10.14.244.101:8081/app/' + id + '/static/upload/' + image
    },
    showDeleteModal (id) {
    ...
    showFormat (id) {
      var link = 'http://10.14.244.101:8081/app/' + id
      if (this.window) {
        this.window.close()
      }
