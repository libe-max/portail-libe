...
// var capitalize = (string) => string.toString().charAt(0).toUpperCase() + string.slice(1)

// var spaceThousand = (number) => number.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ')
// var commaToDot = (number) => number.toString().replace(',', '.')
// var dotToComma = (number) => number.toString().replace('.', ',')

/*
var nonBreakable = (string) => {
  var characters = ['?', '!', ':', ';']
  var i
  for (i = 0; i < characters.length; ++i) string = string.replace(new RegExp('\\s+[' + characters[i] + ']', 'g'), '&nbsp;' + characters[i])
  return string
}
*/
