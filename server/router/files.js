...
var removeAccents = (string) => string.replace(allAccents, (match) => characterMap[match])
var slugify = (string) => _.toString(removeAccents(string)).trim().toLowerCase().replace(/ /g, '-').replace(/([^a-zA-Z0-9_-]+)/g, '')
...
router.post('/social/:type', function(req, res) {
  ...
  let socialFile = req.files.socialFile;

  socialFile.mv('./make/' + req.params.type + '/tmp/static/upload/' + slugify(socialFile.name) + '.' + socialFile.name.split('.').pop(), function(err) {
    if (err)
...
router.post('/upload/:type', function(req, res) {
  ...
  let cardeFile = req.files.cardeFile;

  cardeFile.mv('./make/' + req.params.type + '/tmp/static/upload/' + slugify(cardeFile.name) + '.' + cardeFile.name.split('.').pop(), function(err) {
    if (err)
      return res.status(500).send(err);
      res.send('File uploaded!');
  });
